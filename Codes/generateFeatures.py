...
def gF(args, X, Y):
    ...
    def generateFeatures(kGap, kTuple, x, y):
        ...
        if args.monoMono == 1:
            monoMonoKGap(x, kGap)      #4*(k)*4

        if args.monoDi == 1:
            monoDiKGap(x, kGap)        #4*k*(4^2)

        if args.monoTri == 1:
            monoTriKGap(x, kGap)       #4*k*(4^3)

        ###
        ###

        if args.diMono == 1:
            diMonoKGap(x, kGap)        #(4^2)*k*(4)

        if args.diDi == 1:
            diDiKGap(x, kGap)          #(4^2)*k*(4^2)

        if args.diTri == 1:
            diTriKGap(x, kGap)         #(4^2)*k*(4^3)

        ###
        ###

        if args.triMono == 1:
            triMonoKGap(x, kGap)       #(4^3)*k*(4)

        if args.triDi == 1:
            triDiKGap(x, kGap)         #(4^3)*k*(4^2)
